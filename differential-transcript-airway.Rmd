...
parametric methods even at n=4 samples per group, so likely some of
the excess of calls from DESeq2 could be from transcripts with high 
...
```{r}
y <- computeInfRV(y)
boxplot(log10(mcols(y)$meanInfRV) ~ DESeq2.extra)
```
